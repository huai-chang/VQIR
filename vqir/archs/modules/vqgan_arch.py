...
class VQGANEncoder(nn.Module):
    def __init__(self, *, ch, ch_mult=(1,2,4,8), num_res_blocks,
                 ...
                 z_channels, double_z=True, **ignore_kwargs):
        super().__init__()
        self.ch = ch
        self.temb_ch = 0
...
class VQGANDecoder(nn.Module):
    def __init__(self, *, ch, out_channels, ch_mult=(1,2,4,8), num_res_blocks,
                 dropout=0.0, resamp_with_conv=True, in_channels,
                 z_channels, give_pre_end=False, **ignorekwargs):
        super().__init__()
        self.ch = ch
        self.temb_ch = 0
        self.num_resolutions = len(ch_mult)
...
class VQGAN(nn.Module):
    def __init__(self, in_channels, out_channels, ch, ch_mult, num_res_blocks, 
                 z_channels, double_z, n_embed, embed_dim, beta=0.25, remap=None, sane_index_shape=False):
        super().__init__()

        self.encoder = VQGANEncoder(ch=ch, ch_mult=ch_mult, num_res_blocks=num_res_blocks,
                                    dropout=0.0, resamp_with_conv=True, in_channels=in_channels,
    ...
    def forward(self, x):
        ...
        quant_feat, emb_loss, [_, _, quant_index] = self.quantize(h)

        res['quant_feat'] = quant_feat
        res['quant_index'] = quant_index

        quant = self.post_quant_conv(quant_feat)
        dec = self.decoder(quant)
        res['dec'] = dec
        return res, emb_loss
